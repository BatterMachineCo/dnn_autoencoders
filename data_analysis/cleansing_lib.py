...
#         axes[feature_idx].set_xlabel("Data Points with Interpolation")
...
# def plot_inp_cycles(reshaped_data, 
#                     cell_idx, feature_names=['Qd', 'Qc', 'V', 'I', 'T', 'Qdlin', 'Tdlin', 'dQdV']
# ):
#     no_of_cycles, no_of_features, no_of_timeseries = reshaped_data.shape[1:4]
...
#         for cycle in range(no_of_cycles):
#             axes[feature_idx].plot(reshaped_data[cell_idx, cycle, feature_idx, :], color=cycle_colors[cycle])
#         #axes[feature_idx].set_title(f"Feature: {feature_name}", fontsize=16)  # Adjust title font size here
#         #axes[feature_idx].set_xlabel("Data Points with Interpolation", fontsize=20)  # Adjust xlabel font size here
#         axes[feature_idx].set_ylabel(feature_name, fontsize=20)  # Adjust ylabel font size here
#                 # Set the tick label sizes for both axes
#         axes[feature_idx].tick_params(axis='x', labelsize=20)
#         axes[feature_idx].tick_params(axis='y', labelsize=20)
#         #axes[feature_idx].grid(True)

#     sm = plt.cm.ScalarMappable(cmap=plt.cm.coolwarm, norm=plt.Normalize(vmin=0, vmax=no_of_cycles-1))
#     sm.set_array([])
#     cbar = fig.colorbar(sm, ax=axes, orientation='vertical', pad=0.01)
#     cbar.set_label('Cycle', fontsize=20)      # Set the label font size
#     cbar.ax.tick_params(labelsize=20)           # Set the tick label font size
...
def cycle_color_map(no_of_cycles):
    """Simple color map function."""
    cmap = plt.cm.get_cmap('coolwarm')
    norm = plt.Normalize(vmin=0, vmax=no_of_cycles-1)
    return cmap(norm(np.arange(no_of_cycles)))
...
def cycle_color_map(no_of_cycles):
    """
    Returns an array of RGBA colors using the 'coolwarm' colormap.
    """
    color_map = plt.cm.get_cmap('coolwarm')
    color_norm = plt.Normalize(vmin=0, vmax=no_of_cycles - 1)
    return color_map(color_norm(np.arange(no_of_cycles)))
import numpy as np
import math
import matplotlib.pyplot as plt
from mpl_toolkits.axes_grid1.inset_locator import inset_axes
...
def plot_inp_cycles_fixed_inset(
    reshaped_data,
    cell_idx,
    feature_names=['Qd', 'Qc', 'V', 'I', 'T', 'Qdlin', 'Tdlin', 'dQdV'],
    x_zoom_ranges=None, 
    insets_positions=None,
    fixed_inset=True,
    set_y_auto=True,
    extra_y_margin=0.1
):
    ...
    for feature_idx, feature_name in enumerate(feature_names):
        ...
        for cycle in range(no_of_cycles):
            ax.plot(
                reshaped_data[cell_idx, cycle, feature_idx, :],
                color=cycle_colors[cycle]
            )

        ax.set_ylabel(feature_name, fontsize=20)
        ax.tick_params(axis='x', labelsize=20)
        ax.tick_params(axis='y', labelsize=20)

        # Handle zoom inset
        if (x_zoom_ranges is not None and insets_positions is not None 
            and feature_idx < len(x_zoom_ranges) 
            and feature_idx < len(insets_positions)):

            x_zoom_range = x_zoom_ranges[feature_idx]
            inset_pos = insets_positions[feature_idx]
            if x_zoom_range is not None and inset_pos is not None:
                (x_inset, y_inset, w_inset, h_inset) = inset_pos
                (xmin, xmax) = x_zoom_range
                ...
                if fixed_inset:
                    axins = inset_axes(
                        ...
                        width=w_inset,
                        height=h_inset,
                        bbox_to_anchor=(x_inset, y_inset),
                    ...
                    axins = inset_axes(
                        ...
                        width="100%",
                        height="100%",
                        bbox_to_anchor=(x_inset, y_inset, w_inset, h_inset),
                        bbox_transform=ax.transAxes,
                        loc='lower left'
                    )
                ...
                # Clamp x-range
                xmin = max(0, xmin)
                xmax = min(xmax, no_of_timeseries)
                axins.set_xlim(xmin, xmax)

                # Auto y-limits
                if set_y_auto:
                    y_data = np.concatenate([
                        reshaped_data[cell_idx, cycle, feature_idx, xmin:xmax]
                        for cycle in range(no_of_cycles)
                    ])
                    y_min, y_max = y_data.min(), y_data.max()
                    if y_max > y_min:
                        margin = extra_y_margin * (y_max - y_min)
                    else:
                        margin = 1.0
                ...
                # Draw a dashed rectangle on main plot to show zoom area
                rect = Rectangle(
                    (xmin, y_min - margin),
                    xmax - xmin,
    ...
    # Hide any extra subplots if feature_names < rows*cols
    for j in range(len(feature_names), len(axes)):
        axes[j].set_visible(False)

    # ----- Create the colorbar in a dedicated axis -----
    # Adjust the main figure so there's room on the right
    plt.subplots_adjust(right=0.88)  # 0.88 leaves space on the right for the colorbar
    # Create a colorbar axis [left, bottom, width, height]
    cbar_ax = fig.add_axes([0.90, 0.12, 0.02, 0.75])  # tweak as needed
    ...
    cbar.set_label('Cycle', fontsize=20)
